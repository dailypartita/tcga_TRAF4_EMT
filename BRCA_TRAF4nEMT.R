...
rna = as.matrix(read.table("BRCA.rnaseqv2__illuminahiseq_rnaseqv2__unc_edu__Level_3__RSEM_genes_normalized__data.data.txt", header=T, row.names=1, sep="\t"))
rna = rna[-1,]
...
data = ifelse(data < -4, -4, data)
pdf(file = "BRCA_TRAF4_EMT_zscore_heatmap.pdf", width = 10, height = 10)
heatmap.2(data,
          Colv = NULL,
          Rowv = NULL,
          trace="none",
...
pdf(file = "BRCA_TRAF4_EMT_zscore_cor.pdf", width = 10, height = 10)
